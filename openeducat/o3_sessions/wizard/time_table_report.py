...
class SessionReport(models.TransientModel):
    ...
    def gen_time_table_report(self):
        template = self.env.ref(
                'o3_sessions.o3_teacher_timetable_generate')
        data = self.read(
            ['start_date', 'end_date', 'course_id', 'batch_id', 'state'])[0]
        if data['state'] == 'student':
            ...
            template = self.env.ref(
                'o3_sessions.o3_teacher_timetable_generatee')
        else:
